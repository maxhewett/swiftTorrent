...
typedef struct {
    ...
    int32_t  state;              // libtorrent torrent_status::state_t (int)
...
// After calling st_get_torrents(), retrieve values for index [0..<count]
const char* st_get_torrent_name(STSessionRef session, int32_t index);

// Stable ID for SwiftUI row identity (info-hash in hex; valid until next st_get_torrents)
